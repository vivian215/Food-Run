...
public class NewLevelOne implements GameLevel {
    ...
    public NewLevelOne (Context ctxt, int screenX, int screenY, final Resources res, GameView gameView, Activity activity) {
        ...
        gameItems = new ArrayList<GameItem>() {
            {
                ...
                add (new Ingredient(res, "flour", 1450, Constants.INGREDIENT_INIT_Y));
                //add (new Ingredient(res, "purple", 1370, Constants.INGREDIENT_INIT_Y));
                for (int i = 0; i < 5; i++) {
                    add (new Coin(res, i*Constants.COIN_SPACE + 1700));
                }
                add(new Spike(res, 2400));
                add (new Ingredient(res, "choc", 2350, Constants.INGREDIENT_INIT_Y - 170));
                for (int i = 0; i < 3; i++) {
                    add (new Coin(res, i*Constants.COIN_SPACE + 2600));
                }
                //add (new Ingredient(res, "flour", 3100, Constants.INGREDIENT_INIT_Y));

                add (new Ingredient(res, "purple", 3050, Constants.INGREDIENT_INIT_Y));
                for (int i = 0; i < 5; i++) {
                    add (new Coin(res, i*Constants.COIN_SPACE + 3300));
                }
                for (int i = 0; i < 2; i++) {
                    add (new Spike(res, i*Spike.getWidth() + 4400));
                }
                add (new Ingredient(res, "pink", 4380, Constants.INGREDIENT_INIT_Y-170));
                add (new Ingredient(res, "blue", 4600, Constants.INGREDIENT_INIT_Y));
                for (int i = 0; i < 2; i++) {
                    add (new Coin(res, i*Constants.COIN_SPACE + 4800));
                }
                add (new Ingredient(res, "green", 5050, Constants.INGREDIENT_INIT_Y));
                add (new Spike(res, 5300));
                add (new Ingredient(res, "white", 5400, Constants.INGREDIENT_INIT_Y));
                add (new Ingredient(res, "orange", 5650, Constants.INGREDIENT_INIT_Y));
                add (new Spike(res, 5900));
                add (new Ingredient(res, "yellow", 6000+Spike.getWidth()/4, Constants.INGREDIENT_INIT_Y - 170));
                add(new Flag(res, 6700));
            }
        };
    ...
    public void draw() {
        ...
        if (gameView.getHolder().getSurface().isValid()) {
            ...
            for (int i = 0; i < gameItems.size(); i++) {
                ...
                Log.d("lag", "gameitem " + item.getType());
            }
